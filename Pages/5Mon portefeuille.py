...
def calculate_global_score(signals):
    score = 0
    total_indicators = len(signals)
    
    for signal in signals:
        if signal['Action'] == 'Acheter':
            score += 1
        elif signal['Action'] == 'Vendre':
            score -= 1

    # Normaliser le score pour obtenir une valeur sur 100
    normalized_score = ((score + total_indicators) / (2 * total_indicators)) * 100
...
with stats_cle_col:
    ...
    # Utilisation des onglets pour différents indicateurs
    tab1, tab2, tab3 = st.tabs(["Fondamentaux","Actionnaires","Autres"])
    
    def highlight_action(val):
        color = 'red' if val == 'Vendre' else 'blue' if val == 'Acheter' else 'green'
    ...
    with tab1:
        if 'uploaded_file' in st.session_state and action:
            # Charger les données de la feuille correspondante à l'action sélectionnée
            sheet_data = pd.read_excel(st.session_state['uploaded_file'], sheet_name=action, header=None)

            # Fonction pour convertir les valeurs en flottants
            def to_float(val):
            ...
            # Sélectionner les lignes et colonnes appropriées et convertir les valeurs
            table_data = sheet_data.iloc[1:8, 7:12].applymap(to_float)
            
            # Ajouter une colonne 'Action' avec des valeurs calculées
            avg_years = table_data.iloc[:, :4].mean(axis=1)
            value_2023 = table_data.iloc[:, 4]
            decision = ['Vendre' if value < avg else 'Acheter' if value > avg else 'Neutre' for value, avg in zip(value_2023, avg_years)]

            # Ajouter la colonne 'Action'
            table_data['Action'] = decision

            # Définir les en-têtes des colonnes
            table_data.columns = ['2019', '2020', '2021', '2022', '2023', 'Action']

            # Définir les en-têtes des lignes
            table_data.index = ['Chiffre d\'affaires', 'Croissance CA', 'Résultat net', 'Croissance RN', 'BNPA', 'PER', 'Dividende']

            # Appliquer un style personnalisé au tableau
            styled_table = table_data.style.map(highlight_action, subset=['Action']).format(format_value).set_table_styles(
                [{'selector': 'th', 'props': [('font-size', '16px'), ('color', 'black'), ('background-color', '#f7f7f7')]},
                {'selector': 'td', 'props': [('font-size', '14px')]}]).set_properties(**{'text-align': 'center'}).set_caption("Action")
            st.markdown('Les chiffres sont en millions de FCFA')
    ...
    with tab2:
        if 'uploaded_file' in st.session_state and action:
            ...
            # Créer le pie chart
            fig = px.pie(pie_data, names='Actionnaire', values='Pourcentage', title='Répartition des Actionnaires', height=440, width=300)
...
# Ajouter la section de recommandation générale
st.markdown("<div class='section-title'>📊 Recommandation Générale</div>", unsafe_allow_html=True)

# Calculer le score global
score_global = calculate_global_score(signals)

# Créer et afficher le graphique du score global
...
# Add custom labels
fig_score.add_annotation(x=0.17, y=0.5, text="Strong Sell", showarrow=False, font=dict(size=12, color="red"))
fig_score.add_annotation(x=0.32, y=0.1, text="Sell", showarrow=False, font=dict(size=12, color="orange"))
fig_score.add_annotation(x=0.5, y=0.02, text="Neutral", showarrow=False, font=dict(size=12, color="yellow"))
fig_score.add_annotation(x=0.68, y=0.1, text="Buy", showarrow=False, font=dict(size=12, color="lightgreen"))
fig_score.add_annotation(x=0.83, y=0.5, text="Strong Buy", showarrow=False, font=dict(size=12, color="green"))

st.plotly_chart(fig_score, use_container_width=True)

# Recommandation basée sur le score global
recommendation = "Acheter" if score_global > 60 else "Vendre" if score_global < 40 else "Neutre"

# Afficher la recommandation
st.markdown(f"""
<div class='executive-summary'>
    <h2 class='exec-summary-title'>Résumé de la Recommandation</h2>
    <p class='exec-summary-content'>
        <strong>Action :</strong> {action} <br>
        <strong>Recommandation Générale :</strong> <span style='color: {"green" if recommendation == "Acheter" else "red" if recommendation == "Vendre" else "orange"};'>{recommendation}</span> <br>
        <strong>Raison Principale :</strong> Basé sur une analyse approfondie des indicateurs techniques, fondamentaux et des méthodes d'évaluation, la recommandation est   {recommendation.lower()} .
    </p>
</div>
""", unsafe_allow_html=True)